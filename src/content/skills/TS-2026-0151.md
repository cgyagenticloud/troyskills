...
- **MCP security vulnerabilities disclosed** (2025) — Multiple security researchers documented OAuth-related vulnerabilities in MCP server implementations, including token theft and consent bypass scenarios. [Invariant Labs MCP research](https://invariantlabs.ai/blog/mcp-security-notification-tool-poisoning-attacks)
- **OAuth phishing via malicious apps** — The general pattern mirrors Google/Microsoft OAuth consent phishing attacks used in major breaches (e.g., the 2020 SolarWinds campaign used OAuth app registration for persistence).

## Real-World Relevance

As MCP adoption grows in 2026, the attack surface of OAuth-based tool authentication expands. Multiple security researchers have demonstrated proof-of-concept MCP server credential theft. The pattern mirrors historical OAuth phishing attacks but is amplified by agent automation—users may not see or review the OAuth consent screen when agents handle authentication programmatically.
