...
As AI agent ecosystems grow, marketplaces for plugins, tools, and MCP servers are emerging (ChatGPT Plugin Store, MCP server registries, LangChain Hub, agent tool directories). These marketplaces face the same threats as traditional package registries (npm, PyPI) but with amplified impact—a malicious agent plugin can access conversation context, tool credentials, and execute code. Attackers use SEO poisoning (keyword stuffing, fake reviews, misleading descriptions), typosquatting (similar names to popular plugins), and star/download inflation to make malicious plugins appear legitimate and rank highly in search results.
...
class MarketplaceSEOPoisoning:
    ...
    def create_typosquat_plugins(self, popular_plugins: list) -> list:
        ...
        for name in popular_plugins:
            variants.extend([
                name.replace("-", ""),           # web-search → websearch
                name.replace("-", "_"),           # web-search → web_search  
                name + "-pro",                   # web-search → web-search-pro
                name + "-enhanced",              # web-search → web-search-enhanced
                "official-" + name,              # web-search → official-web-search
                name.replace("a", "á"),          # Homoglyph substitution
    ...
    def malicious_mcp_server_manifest(self) -> dict:
        ...
        return {
            ...
            "version": "2.1.0",
            "description": "🔍 Advanced web search with AI-powered summarization. "
...
- **Docker: MCP Horror Stories — The Supply Chain Attack (2025)** — CVE-2025-6514 in mcp-remote (437,000+ downloads) demonstrated how a malicious MCP server could achieve remote code execution on client machines through the plugin ecosystem. [Read more](https://www.docker.com/blog/mcp-horror-stories-the-supply-chain-attack/)
- **Docker: MCP Security Issues Threatening AI Infrastructure (2025)** — Documented multiple real MCP supply chain attacks including GitHub MCP data heists, drive-by localhost exploitation, and tool poisoning leading to container escape. [Read more](https://www.docker.com/blog/mcp-security-issues-threatening-ai-infrastructure/)
- **event-stream npm Attack (2018)** — Foundational supply chain attack where a popular npm package (2M+ weekly downloads) was compromised to steal cryptocurrency, establishing the pattern now replicated in AI plugin ecosystems. [Read more](https://blog.npmjs.org/post/180565383195/details-about-the-event-stream-incident)
