...
- **ChatGPT Hex Encoding Jailbreak (2024)** — Researcher Marco Figueroa demonstrated that encoding instructions as hexadecimal text caused ChatGPT-4o to decode and execute them, bypassing safety filters — a classic cross-modal transfer from text encoding to code generation to action. [Read more](https://www.theregister.com/2024/10/29/chatgpt_hex_encoded_jailbreak/)
- **Pillar Security: MCP Cross-Server Data Leakage (2025)** — Demonstrated how text-based prompt injection in one MCP tool could cause the agent to generate code that exfiltrates data through another tool, showing the text→code→action pipeline. [Read more](https://www.pillar.security/blog/the-security-risks-of-model-context-protocol-mcp)
