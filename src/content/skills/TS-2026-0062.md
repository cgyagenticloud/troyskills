...
- **Claude Desktop** — Remote MCP servers connected via SSE/HTTP are vulnerable to DNS-based MITM
- **OpenClaw** — Gateway connections to remote MCP servers rely on DNS resolution
- **Any MCP client using remote servers** — The MCP protocol over HTTP/SSE lacks built-in server authentication

**At-Risk MCP Servers/Tools:**
- Remote MCP servers accessed via domain names (not localhost)
- `mcp-remote` — npm package for remote MCP connections shown to be vulnerable
- Cloud-hosted MCP servers without certificate pinning

**Related Incidents:**
- [INC-004: MCP-Remote RCE via Malicious Authorization Endpoint](/incidents) — Demonstrated remote MCP server exploitation
- General DNS hijacking incidents (e.g., 2024 Sitting Ducks attacks affecting millions of domains)
- OWASP API Security — Server-side request forgery patterns applicable to MCP remote connections
